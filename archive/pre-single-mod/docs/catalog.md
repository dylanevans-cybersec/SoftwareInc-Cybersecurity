...
**Scope:** data (TyD) in `Mods\Cybersecurity` plus a C# incident system in `DLLMods\Cybersecurity` (docs\incidents.md). They are two separate folders, each linked into the game by `tools\link.ps1`. An attempt to bundle the data inside the code mod (loaded by the code through `GameData.LoadSteamMod`) did not work in the game and was reverted. Furniture was abandoned. There is no SOC-analyst specialization from data: custom specs are rejected by the game, so the Cybersecurity specialisation is added by the code (docs\incidents.md).
