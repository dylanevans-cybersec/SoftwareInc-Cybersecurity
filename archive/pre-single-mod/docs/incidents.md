...
The name is localized by the `Cybersecurity` key in `Mods\Cybersecurity\Localization\English\UI.tyd`. The per-level tooltip lines (`SpecDesc > Service > ...`) are not provided, because a mod file with a `SpecDesc` table might replace the vanilla one.
